...
export class Guard {
  ...
  public static againstZeroOrUndefined (argument: any, argumentName: string): IGuardResult {

    try {
      if (argument === null || argument === undefined) {
        return { succeeded: false, message: `${argumentName} is null or undefined` }
      } 
      let value =parseFloat(argument).toFixed(2);
      if(value) {
        if(parseFloat(value) <= 0.00) {
          return { succeeded: false, message: `${argumentName} should be greater than 0.0` }
        }
      }else{
        return { succeeded: false, message: `${argumentName} has invalid number` }
      }

      return { succeeded: true }
    }
    catch (err) {
      return { succeeded: false, message: err.message }
    }
